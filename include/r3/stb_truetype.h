...
#define stbtt_tag1(p,str)           ((p)[0] == str[0])

static int stbtt__isfont(const stbtt_uint8 *font)
{
   // check the version number
   if (stbtt_tag1(font, "1"))   return 1; // TrueType 1
   ...
   if (format == 0) { // apple byte encoding
      ...
      while (low <= high) {
	   ...
	   for( int i = 0; i < ngroups; i++ ) {
		   stbtt_uint32 start_char = ttULONG(data+index_map+16+i*12+0);
		   stbtt_uint32 end_char = ttULONG(data+index_map+16+i*12+4);
		   if( start_char <= (stbtt_uint32)unicode_codepoint && (stbtt_uint32)unicode_codepoint <= end_char ) {
			   stbtt_uint32 start_glyph = ttULONG(data+index_map+16+i*12+8);
			   return start_glyph + unicode_codepoint-start_char;			   
		   }
	   }
      return 0; // not found
